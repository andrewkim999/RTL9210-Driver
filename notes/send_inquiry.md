The very first transfer to be sent will be the inquiry command. The inquiry command is an SCSI command that makes the device identify itself. Inquiry is the simplest possible read operation that we implement before implementing actual read/write commands.
...
Correction: thread 2 is not a regular thread, it's an interrupt context. Maybe I will explain the difference between a process context and interrupt context in another file.. 
